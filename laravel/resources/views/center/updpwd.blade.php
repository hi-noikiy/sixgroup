...
<div class="container_24 clearfix" id="orderWrapper">
    ...
    <div class="grid_19" id="order">
        <div class="orderHead"><h4>修改密码</h4></div>
        <form action="{{URL('updatepwd')}}" autocomplete="off" method="post" onsubmit="return fun()">
            <div class="modifyPassword clearfix">
                <p><label for="">原密码</label><input class="pwd" data-val="true" data-val-length="密码长度不正确" data-val-length-max="16" data-val-length-min="4" data-val-required="请输入旧密码" id="u_pwd" name="u_pwd" type="password" onblur="checkpwd()"/><span class="field-validation-valid" data-valmsg-for="OldPassword" id='spwd' data-valmsg-replace="true" style="color:red;font-size: 15px" ></span></p>
                <p><label for="">新密码 </label><input class="pwd" data-val="true" data-val-length="密码长度不正确" data-val-length-max="16" data-val-length-min="6" data-val-required="请输入新密码" id="npwd" name="npwd" type="password" onblur="checknnpwd()"/><span class="field-validation-valid" data-valmsg-for="NewPassword" id="nnpwd" data-valmsg-replace="true" style="color:red;font-size: 15px"></span></p>
                <p><label for="">重复新密码</label><input class="pwd" data-val="true" data-val-equalto="两次输入的新密码不一致" data-val-equalto-other="*.NewPassword" data-val-length="密码长度不正确" data-val-length-max="16" data-val-length-min="6" data-val-required="请重新输入新密码" id="rpwd" name="rpwd" type="password" onblur="checkrpwd()"/><span class="field-validation-valid" data-valmsg-for="RepNewPassword" id='rrpwd' data-valmsg-replace="true" style="color:red;font-size: 15px"></span></p>
            </div>
            <div class="buttonBox"><input name="_token" value="{{csrf_token()}}" type="hidden"/><input type="submit" id="sub" value="确定修改" class="button button03" /></div>
        </form></div>
</div>
...
<script type="text/javascript">
    var staticFileRoot = "http://staticfile.tujia.com",
            ...
            TUJIA_CLIENTID = '63fb654c-3956-42d8-ba4c-868f3bd10a37';
    ...
    traceData.prevId = '';
    traceData.pageId = 'b98267e7-aeb0-401e-8892-81ab30a34672';
    traceData.url = '/UserInfo/ChangePassword/';
    traceData.params = '';
</script>
...
<script type="text/javascript">
    var spans = $(".field-validation-error span");
    if (spans.length == 0) {
...
{{--<script type="text/javascript">--}}
    {{--$("#mypassword").addClass("current");--}}
    {{--(function () {--}}
        {{--if (traceLog) {--}}
            {{--traceLog("Common", {--}}
                {{--pageName: "ChangePasswordIndex",--}}
...
<script>
    ...
    function checkpwd()
    {
        ...
        if (u_pwd == '') {
            document.getElementById('spwd').innerHTML = "原密码不能为空";
    ...
    function checknnpwd()
    {
        ...
        if(npwd=='')
        {
            document.getElementById('nnpwd').innerHTML='新密码不能为空';
            document.getElementById('sub').disabled=true;
            return false;
        }
        else if(n_pwd.test(npwd))
        {
            document.getElementById('nnpwd').innerHTML='';
            document.getElementById('sub').disabled=false;
            return true;
        }
        else
        {
            document.getElementById('nnpwd').innerHTML='新密码必须为6位以上';
            document.getElementById('sub').disabled=true;
            return false;
        }
    }
    ...
    function checkrpwd()
    {
        ...
        if(rpwd!=npwd)
        {
            document.getElementById('rrpwd').innerHTML='与新密码不一致';
            document.getElementById('sub').disabled=true;
            return false;
        }
        else
        {
            document.getElementById('rrpwd').innerHTML='';
            document.getElementById('sub').disabled=false;
            return true;
        }
    }
