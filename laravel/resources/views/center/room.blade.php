<!DOCTYPE html>
<html>
<head>
    <meta http-equiv="Cache-Control" content="no-transform" />
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge">

    <meta name="mobile-agent" content="format=html5; url=http://m.tujia.com/sanya_gongyu/se0/sanyawan_90492.htm">

    <meta name="robots" content="noindex" />

    <link rel="canonical" href="http://www.tujia.com/sanya_gongyu/sanyawan_90492.htm"/>
    <title>【图】三亚瑞家公寓揽海精品两房一厅_三亚湾短租公寓_途家网</title>
    <meta name="description" content="三亚三亚湾海坡开发区中海路5号酒店式公寓,三亚瑞家公寓揽海精品两房一厅,位于三亚三亚湾海坡开发区中海路5号金中海蓝钻小区4号楼,三亚瑞家精品海景公寓位于三亚顶级社区金中海蓝钻小区，地处于三亚市三亚湾核心地段，与绵延约22公里的椰梦...！" />
    <meta name="keywords" content="三亚三亚湾海坡开发区中海路5号酒店式公寓,三亚瑞家公寓揽海精品两房一厅" />
    <meta property="qc:admins" content="27330065376452116375" />
    <meta property="wb:webmaster" content="791d1c6849c2b026" />


    <link rel="shortcut icon" href="http://staticfile.tujia.com/PortalSite/Images/favicon.ico" type="image/x-icon" />
    <link rel="bookmark" href="http://staticfile.tujia.com/PortalSite/Images/favicon.ico" type="image/x-icon" />

    <link rel="stylesheet" type="text/css" href="http://staticfile.tujia.com/portalsite2/styles/css_common?v=405bfef" />
    <link rel="stylesheet" type="text/css" href="http://staticfile.tujia.com/portalsite2/styles/im.css?v=405bfef" />

    <link rel="stylesheet" type="text/css" href="http://staticfile.tujia.com/portalsite2/styles/css_detail?v=405bfef" />

    <script type="text/javascript" src="http://staticfile.tujia.com/portalsite2/scripts/base/jquery.js?v=405bfef"></script>
    <script type="text/javascript" src="http://staticfile.tujia.com/portalsite2/scripts/js_common_head?v=405bfef"></script>

    <script type="text/javascript">
        //window["WEB_XHR_POLLING"] = true;
        var MESSAGE_RADIO="http://staticfile.tujia.com/PortalSite2/radio/message.wav", ORDERNOTICE_RADIO = "http://staticfile.tujia.com/PortalSite2/radio/ordernotice.wav";
    </script>

    <script src="http://www.tujia.com/Common/CityInfo/"></script>

    <script type="text/javascript">
        var staticFileRoot = "http://staticfile.tujia.com",
                minDate = "2016-05-19",
                mindate =  new Date(2016,4,19),
                maxDate = "2016-11-15",
                maxdate= new Date(2016,10,15),
                houseId,
                ServerDomain = "tujia.com",
                TUJIA_CLIENTID = '7352267d-d394-44f8-941c-f22daaa0ac1b';

        var portalUrl = "http://www.tujia.com";
        var favoriteUrl = "http://vip.tujia.com";
        var customerUrl =  "http://vip.tujia.com";
        var imUrl = "http://im.tujia.com";


        var traceData = window.traceData || {};
        traceData.logService = 'http://api.tujia.com';
        traceData.prevId = '';
        traceData.pageId = '27abdd25-9fe8-4bd9-879b-f81d54d8c858';
        traceData.url = 'http://www.tujia.com/sanya_gongyu/se0/sanyawan_90492.htm';
        traceData.params = '';
    </script>


    <script type="text/javascript" src="http://staticfile.tujia.com/Portalsite2/Scripts/Base/jquery-1.11.3.min.js"></script>
    <script type="text/javascript">
        var $jq = jQuery.noConflict(true);
    </script>
    <script type="text/javascript" src="http://staticfile.tujia.com/Portalsite2/Scripts/Base/owl.carousel.js"></script>
    <script type="text/javascript" src="http://staticfile.tujia.com/portalsite2/scripts/js_detail_head_v2?v=405bfef"></script>
    <script type="text/javascript">

        var Tj = Tj || {};
        Tj.Detail = Tj.Detail || {};
        // 是否需要加载产品
        Tj.Detail.LoadProduct =1;
        // 是否需要初始化入离日期
        Tj.Detail.InitDatePicker=1;
        // 是否需要加载地图
        Tj.Detail.LoadMap=1;
        // ABTest
        Tj.ABTest='A';
        // Mock
        //Tj.ABTest="B";

        Tj.Detail.Id=90492;

        Tj.Detail.Subtype=0;

        Tj.Detail.CurrentUrl='/sanya_gongyu/se0/sanyawan_90492.htm';

        Tj.Detail.IsSweetome=0;

        Tj.Detail.HotelId=25587;

        Tj.Detail.C2CHotelId=25587;

        var gPrevStartDate;
        var gPreEndDate;

        var getProductUrl= "/UnitDetail/UnitProductPlus/" ;

        function txtStartDateChangeHandle(gtxtStartDate, gtxtEndDate) {
            var vtmpStartDate=gtxtStartDate.val();
            var vtmpEndDate=gtxtEndDate.val();
            if (!vtmpStartDate && !vtmpEndDate) {
                return;
            }
            if (!vtmpEndDate) {
                var tmpDate = new Date(vtmpStartDate);
                tmpDate.setDate(tmpDate.getDate()+1);
                vtmpEndDate=tmpDate.Format("yyyy-MM-dd");
            }
            if (vtmpStartDate == gPrevStartDate) {
                return;
            }
            gPrevStartDate = vtmpStartDate;
            gPreEndDate=vtmpEndDate;
            //setSDFlag();
            //syncCalender();
        }
        function txtEndDateChangeHandle(gtxtStartDate, gtxtEndDate) {
            var vtmpStartDate=gtxtStartDate.val();
            var vtmpEndDate=gtxtEndDate.val();
            if (!vtmpStartDate && !vtmpEndDate) {
                return;
            }
            if (!vtmpStartDate) {
                var tmpDate = new Date(vtmpEndDate);
                tmpDate.setDate(tmpDate.getDate()-1);
                vtmpStartDate=tmpDate.Format("yyyy-MM-dd");
            }

            if (vtmpEndDate == gPreEndDate) {
                return;
            }
            gPrevStartDate = vtmpStartDate;
            gPreEndDate=vtmpEndDate;
            //setSDFlag();
        }
        var mapType=1;

        var isLogin = true;

        var staticFileRoot = "http://staticfile.tujia.com",
                minDate = "2016-05-19",
                mindate =  new Date(2016,4,19),
                maxDate = "2016-11-15",
                maxdate= new Date(2016,10,15),
                houseId,
                ServerDomain = "tujia.com",
                TUJIA_CLIENTID = '02b85793-53f9-4db6-a7dd-e6ff4a277adb';

        var portalUrl = "http://www.tujia.com";
        var imUrl = "http://im.tujia.com";
        var customerUrl =  "http://vip.tujia.com";

    </script>

</head>
<body id=&#39;tujia&#39;>
<!--[if lt IE 8]>
<div class="ie-tips">
    <span> 您使用的IE浏览器版本较低！</span>本站已不再支持较低版本的IE浏览器，已为您启用了精简版。为了更好的体验本站内容，建议您升级<a target="_blank" href="http://www.microsoft.com/zh-cn/download/ie.aspx?q=internet+explorer">Internet Explorer浏览器</a>或安装非IE内核浏览器。请下载 <a href="http://down.360safe.com/se/360se7.1.1.556.exe" class="link-btn" target="_blank">360浏览器</a>或<a href="http://dldir1.qq.com/invc/tt/QQBrowser_Setup_Wireless.exe" class="link-btn" target="_blank">QQ浏览器</a>。
</div>
<![endif]-->
<!--VISITOR-81-36-->
<input id="hdIsFromPartner" name="hdIsFromPartner" type="hidden" value="0" />


<div id="J-ad-top" class="event-ad-fluid ">
    <a href="http://go.tujia.com/1525/?code=20160519nc1sytl" target="_blank" style="background:url(http://staticfile.tujia.com/upload/specialOffer/day_160518/201605182122449913.jpg) no-repeat top center" >
    </a>
</div>


<div class="hd-wrap-fluid ">
    <div class="m-hd">
        <div class="site-logo-cont">
            <img style="margin-top: 10px;margin-left: 17px" height="40px" width="100px" src="images/logo.jpg" alt="蜗牛logo">
            <span class="logo-text">旅途中的家</span>
        </div>
        <div class="nav-list">
            <ul>
                <li class="first"><a href="http://www.tujia.com/se0/" class="home-link " target="_blank">首页</a></li>
                <li><a href="http://international.tujia.com/"  target="_blank">海外·港台</a></li>
                <li><a href="http://www.tujia.com/changzu/se0/"  target="_blank">周租月租</a></li>
                <li><a href="http://www.tujia.com/Experience/"  target="_blank">体验房</a></li>
                <li><a href="http://www.tujia.com/photowall/" target="_blank" >特色房</a></li>

                <li><a href="http://travel.tujia.com" target="_blank">社区</a></li>
                <li><a href="http://www.tujia.com/EntrustTreasure/"  target="_blank">买楼收租</a></li>
                <li><a href="http://www.tujia.com/TuYuan/"  target="_blank">途远度假</a></li>
            </ul>
        </div>

        <div class="sub-link"><a href="{{URL('center')}}">我的蜗牛</a></div>
    </div>
</div>


<div class="m-cont row-cont">


    <div id="dir" class="dir-cont">
        <a href="http://www.tujia.com/se0/" title="途家网">途家网</a>

        <em>&gt;</em><a href="/sanya_gongyu/se0/" title="三亚住宿">三亚住宿</a>
        <em>&gt;</em><a href="/sanya_gongyu/sanyawan_se0s2/" title="三亚湾住宿">三亚湾住宿</a><em>&gt;</em>

        三亚瑞家公寓揽海精品两房一厅

    </div>


    <div class="share-cont">
        <a class="a-btn" target="_blank" href="http://www.tujia.com/Account/BeginShare?state=SinaWeiBo&amp;unitID=90492"><i class="icon-sian"></i>分享到微博</a>
        <a class="a-btn" href="javascript:void(0);" id="shareWeixin"><i class="icon-wixin"></i>分享到微信</a>
        <a class="addFavorite a-btn last-btn " id="favorite" data-page="unitdetail" onclick="_gaq.push(['_trackEvent', 'pcDetail', '点赞']);" unitid="90492" href="javascript:void(0);">
            <i class="icon-chevron"></i>
            <span id="txtdianzan">我想住</span>
        </a>

    </div>

</div>

<script type="text/javascript">
    var imgTotalCount=13;
    //imgTotalCount=5;
    var imgLoadedCount = 0;

    function imgLoadSuccess() {
        imgLoadedCount++;
        if(imgLoadedCount==imgTotalCount&&imgTotalCount>0)
        {

            initOwlCarousel();
        }
    }

    function imgLoadFail(self)
    {

        imgTotalCount--;
        self = $(self);
        self.parent().remove();
        if (imgLoadedCount == imgTotalCount&&imgTotalCount>0) {
            initOwlCarousel();
        }
    }

    function initOwlCarousel() {
        var carouselDetailPic = $jq('.owl-carousel').owlCarousel({
            //nav:true,
            margin:8,
            loop: true,
            autoWidth: true,
            items: 4
        });
        $jq('#imgControl').show();

        $jq('#hd-next').click(function () {
            carouselDetailPic.trigger('next.owl.carousel');
        })

        $jq('#hd-prev').click(function () {
            // With optional speed parameter
            // Parameters has to be in square bracket '[]'
            carouselDetailPic.trigger('prev.owl.carousel');
        });
    }


</script>



<div class="m-cont m-con-fluid  new-cont-fluid">
    <div class="thumb-cont">
        <div class="shadow-box"></div>
        <div class="owl-carousel" style="width: 9999px">
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160505/thumb/201605051733501789_390_390.jpg" />
            </div>
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160422/thumb/201604220934115354_390_390.jpg" />
            </div>
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160422/thumb/201604220934243913_390_390.jpg" />
            </div>
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160422/thumb/201604220936493677_390_390.jpg" />
            </div>
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160226/thumb/201602261153173816_390_390.jpg" />
            </div>
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160422/thumb/201604220934373909_390_390.jpg" />
            </div>
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160226/thumb/201602261153162212_390_390.jpg" />
            </div>
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160422/thumb/201604220934586715_390_390.jpg" />
            </div>
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160422/thumb/201604220936094266_390_390.jpg" />
            </div>
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160422/thumb/201604220933322885_390_390.jpg" />
            </div>
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160422/thumb/201604220938004603_390_390.jpg" />
            </div>
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160422/thumb/201604220938399414_390_390.jpg" />
            </div>
            <div class="item">
                <img onload="javascript:imgLoadSuccess();" onerror="javascript:imgLoadFail(this);" src="http://pic.tujia.com/upload/landlordunit/day_160328/thumb/201603281239409654_390_390.jpg" />
            </div>
        </div>
        <div id="imgControl" class="thumb-btn" style="display:none">
            <a href="javascript:;" class="btn-prev" id="hd-prev">上一张</a>
            <a href="javascript:;" class="btn-next" id="hd-next">下一张</a>
        </div>
    </div>



    <div class="house-hint-info">
        <div class="house-tag-box">

            <span class="label-text">家庭出游</span>
            <span class="label-text">绿化优美</span>
            <span class="label-text">阳光沙滩</span>

        </div>
        <span>此房型有3套，实际入住与此可能略有不同</span>
    </div>
</div>


<div class="m-share-pop" style="display: none;" id="divWeixin">
    <div class="hd-box">
        <h2>分享到微信</h2>
        <a href="javascript:;" title="关闭" class="close-btm" id="closeWeixin"></a>
    </div>
    <div class="cont-box">
        <h2>
            用微信“扫一扫”
            <br />
            将心动的房屋分享给自己的好友和朋友圈吧！
        </h2>
        <p class="code-box" id="wechatImg"></p>
        <p class="text-info">
            打开微信，点击底部的“发现”，使用 “扫一扫” 即可将网页分享<br />
            给我的好友和朋友圈。
        </p>
    </div>
</div>

<script type="text/javascript">
    $("#shareWeixin").overlay({
        mask: {
            color: '#000',
            loadSpeed: 200,
            opacity: 0.6
        },
        // onBeforeLoad: function () {   },
        close: "#closeWeixin",  //触发关闭的dom节点，可以是图片，链接或者按钮
        closeOnClick: true, // 点击弹出层之外位置是否关闭弹出层
        closeOnEsc: true,   // 按下ESC是否关闭弹出层
        target: "#divWeixin",
        top: "20%",
        onBeforeLoad: function () {
            if ($("#divWeixin").attr("loaded") == "1")
                return;
            $.getScript('http://staticfile.tujia.com/PortalSite2/Scripts/weixinQrCode.js', function () {
                var href = window.location.href;
                if (href.indexOf("?") > 0) href = href.substr(0, href.indexOf("?"));
                if (href.indexOf("#") > 0) href = href.substr(0, href.indexOf("#"));
                QRCode.wechatQr.makeCode(href);
                $("#divWeixin").attr("loaded", "1");
            });
        }
    });
</script>



<div class="m-cont m-con-fluid">
    <div class="m-hotel-info">
        <div class="hotel-cont">
            <div class="room-info">
                <h1>
                    三亚瑞家公寓揽海精品两房一厅<span class="personal-tag landlord-tag" title="个人房屋分享，最有家的感觉">个人房东</span>

                    <a href="http://bp.tujia.com/2016/SelectMerchants" target="_blank" class="icon-quality-hotel" rel="PreferredUnitTips"></a>
                </h1>
                <div class="district-info">
                    <i class="icon-area"></i>

                    <span class="lab-text">三亚三亚湾海坡开发区中海路5号金中海蓝钻小区4号楼</span>

                    [
                    近三亚湾

                    ]

                    <a href="#unitmap" class="link-btn">查看地图</a>
                </div>
                <div class="hotel-advan-tag">
                    <ul>
                        <li class="icon-type" title="普通公寓">普通公寓</li>
                        <li class="icon-square" title="建筑面积65平米">2室1厅1卫</li>
                        <li class="icon-cooking" title="可做饭">可做饭</li>
                        <li class="icon-bed" title="2张床"><span class="lab-text"><u rel="bedCountDetail">2张床</u></span></li>
                        <li class="icon-guests" title="推荐入住4人">宜住4人</li>
                        <li class="icon-seaview" title="海景">海景</li>

                        <li class="icon-wiff" title="无线网络">无线网络</li>
                    </ul>

                    <span id="bedCountDetail" class="m-tips-wrap" style="display:none;padding:5px;">2×1.8米 大床×1<br>2×1.5米 大床×1</span>
                </div>
            </div>
            <div class="hotel-sidebar">
                <div class="book-info ">
                    <div id="bestProduct" class="price-info">
                        <div class="room-price">
                            <span>加载中...</span>
                        </div>
                    </div>
                    <div class="comment-info">
                        <span id="unitscore" rel="unitCommentSummaryTooltip" class="rating-box"><b class="hotel-value">5</b>/5分</span>
                        <a id="commentsAreaLink" rel="nofollow" class="comments-area" style="cursor: pointer;" onclick="_gaq.push(['_trackEvent', 'pcDetail', '房屋点评']);"><span class="comments-count">40</span>条点评</a>
                        <span rel="j-experience-info" class="exp-voucher j-bestproduct-experience">（发点评赢体验券<i class="icon-desc-text"></i>）</span>
                    </div>
                    <div class="m-tips-wrap dn" id="unitCommentSummaryTooltip">
                        <div class="tips-content comments-total">
                            <h2>房屋综合评分</h2>
                            <div class="results-wrap">
                                <div class="star-box star" data-rating="5"></div>
                                <div class="results-comments">
                                    <span class="hotel-value">5</span>/5分
                                </div>
                            </div>
                            <div class="comments-items">
                                <ul>
                                    <li><span class="comments-items-name">整洁卫生：</span><span class="number-box color-blue">5</span>分</li>
                                    <li><span class="comments-items-name">交通位置：</span><span class="number-box color-blue">5</span>分</li>
                                    <li class="border-none"><span class="comments-items-name">管理服务：</span><span class="number-box color-blue">5</span>分</li>
                                    <li class="border-none"><span class="comments-items-name">设施装修：</span><span class="number-box color-blue">5 </span>分</li>
                                </ul>
                            </div>
                        </div>
                    </div>

                    <div id="travelsummary" class="travel-info" style="display:none"><a href="#experience"><i class="icon-travel"></i></a></div>
                </div>
            </div>
        </div>
    </div>
</div>

<!-- 开始    2015-10-19  -->
<div class="m-cont t-cont clearfix">
    <div id="unitmaincontainer" class="main-cont ">
        <!-- 产品  -->
        <div class="m-column-box new-column-box" id="saleproductsection">
            <div class="product-filter" id="productfilter">
                <div class="date-change-box">
                    <div class="change-control-group">
                        <span class="controls-span">入住</span>
                        <div class="controls">
                            <input type="hidden" id="UnitId" value="90492" />
                            <input type="hidden" id="ProductId" value="0" />
                            <input type="hidden" id="UnitDetailSubType" value="0" />
                            <label for="unitDetailStartDate">
                                <input type="text" name="" id="unitDetailStartDate" value="2016-05-19" class="date-ipt-text" /><i class="icon-date"></i>
                            </label>
                        </div>
                    </div>

                    <div class="change-control-group">
                        <span class="controls-span">退房</span>
                        <div class="controls">
                            <label for="unitDetailEndDate">
                                <input type="text" name="" id="unitDetailEndDate" value="2016-05-20" class="date-ipt-text" /><i class="icon-date t-icon-date"></i>
                            </label>
                        </div>
                    </div>
                    <div class="change-control-group btn-group">
                        <div class="controls"><a href="{{URL('orderadd')}}?r_id={{$v->r_id}}" id="changeBtn" class="change-btn">确定</a></div>

                    </div>
                    <div class="room-status">
                        <a class="has-empty-house" href="javascript:;" data-unitid="90492" data-detaillink="" onclick="_gaq.push(['_trackEvent', 'pcDetail', '哪天有房']);">哪天有房</a>
                    </div>
                </div>

                <div class="hint-text">



                    <div class="m-tips-wrap" id="noReturnTips" style="display: none;">
                        <div class="tips-content ">
                            <div class="tips-info">

                                <p style="list-style: none; text-align: left">您现在还是普通会员，暂不能享受高级会员额外优惠，加油！</p>

                            </div>
                        </div>
                    </div>
                    <span rel="noReturnTips" id="J-return-tips">非高级会员暂不享<a href="http://vip.tujia.com/Memberhome/ReturnCash" target="_blank" class="link-btn">额外优惠</a></span>
                </div>

            </div>
            <div class="room-select-box" id="productcontent">
                <div class="loading-wrap">
                    <i class="icon-loading"></i>
                </div>
            </div>
        </div>
        <!-- Tabs  -->


        <div class="m-column-box ">
            <div id="HouseTabs" class="house-tabs-box">
                <div id="contentTab" class="house-tabs ">
                    <ul>

                        <li id="unitdetailinfotab" class="current item-first">
                            <a href="#unitdetailinfo">房屋详情</a>
                        </li>
                        <li>
                            <a id="unitcheckinneedtoknowtab" href="#unitcheckinneedtoknow" onclick="_gaq.push(['_trackEvent', 'pcDetail', '入住须知']);">入住须知</a>
                        </li>
                        <li>
                            <a id="unitmaptab" href="#unitmap" onclick="_gaq.push(['_trackEvent', 'pcDetail', '位置地图']);">位置地图</a>
                        </li>
                        <li id="commentsareatab" class="j-tabs">
                            <a href="#commentsArea" id="usercomments" onclick="_gaq.push(['_trackEvent', 'pcDetail', '点评导航']);">
                                入住点评<span id="usercomments-value" class="total-box">40</span>条
                            </a>
                        </li>
                        <li id="unitgoodexperiencetabcontainer" style="display:none" >
                            <a id="unitgoodexperience" href="#experience" onclick="_gaq.push(['_trackEvent', 'pcDetail', '达人体验']);">达人体验</a>
                        </li>
                        <li class="house-tabs-last">
                            <a rel="nofollow" id="tabbookprice" href="#productfilter" style="padding: 0; vertical-align: -4px" onclick="_gaq.push(['_trackEvent', 'pcDetail', '置顶条查看房价']);"><span id="pricebox" class="price-box dn"></span></a>
                            <a rel="nofollow" id="tabbookbtn" class="book-btn" href="#productfilter" onclick="_gaq.push(['_trackEvent', 'pcDetail', '置顶条查看房价']);">
                                查看房价
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>

        <!-- 房屋详情  -->
        <div class="m-column-box t-column-box" id="unitdetailinfo">
            <!-- 房屋亮点  -->



            <!-- 设施服务  PS:设施与服务数据合并  -->

            <div class="column-info">
                <div class="side-box">
                    <i class="icon-facility"></i>
                    <h2>设施服务</h2>
                </div>
                <div class="content-box">
                    <div id="facilitycontainer" class="m-key-list">
                        <h4>设施</h4>
                        <ul style="max-height:79px;">
                            <li class="icon-wifi ">无线网络</li>
                            <li class="icon-hotwater ">全天热水</li>
                            <li class="icon-tv ">电视</li>
                            <li class="icon-airconditioner ">空调</li>
                            <li class="icon-icebox ">冰箱</li>
                            <li class="icon-washingmachine ">洗衣机</li>
                            <li class="icon-kettle ">电水壶</li>
                            <li class="icon-towel ">毛巾</li>
                            <li class="icon-toothbrush ">牙具</li>
                            <li class="icon-hairandbodylotion ">洗发水/沐浴露</li>
                            <li class="icon-hairdryer ">电吹风</li>
                            <li class="icon-slipper ">拖鞋</li>
                            <li class="icon-elevator ">电梯</li>
                            <li class="icon-bathtub ">浴缸</li>
                            <li class="icon-broadband ">宽带</li>
                            <li class="icon-heating icon-not">暖气</li>
                            <li class="icon-bathtowel icon-not">浴巾</li>
                        </ul>

                        <h4>服务</h4>
                        <ul style="max-height:35px;">
                            <li class="">行李寄存</li>
                            <li class="dis-text">免费停车</li>
                            <li class="dis-text"></li>
                        </ul>
                    </div>

                    <div id="facilityshowmore" class="toggle-btn"><a href="javascript:;" class="">查看更多<i class="i-acor"></i></a></div>
                    <div id="facilityshowless" class="toggle-btn dn"><a href="javascript:;" class="active">收起<i class="i-acor"></i></a></div>
                </div>
            </div>
            <script type="text/javascript">
                $("#facilityshowmore a").click(function () {
                    $("#facilitycontainer > ul").attr("style", "");
                    $("#facilityshowmore").hide();
                    $("#facilityshowless").show();
                });
                $("#facilityshowless a").click(function () {
                    $("#facilitycontainer > ul").eq(0).css("max-height", "79px");
                    $("#facilitycontainer > ul").eq(1).css("max-height", "35px");
                    $("#facilityshowless").hide();
                    $("#facilityshowmore").show();
                });
            </script>


            <!-- 房屋描述  -->


            <div class="column-info" id="unitintrocontent">
                <div class="side-box">
                    <i class="icon-unitintro"></i>
                    <h2>房屋描述</h2>
                </div>
                <div id="unitintrocontentcontainer" class="content-box">
                    <div class="desc-info " style="">
                        三亚瑞家精品海景公寓位于三亚顶级社区金中海蓝钻小区，地处于三亚市三亚湾核心地段，与绵延约22公里的椰梦长廊直线距离仅为约200米，交通便利，周边配套齐全，依山傍海，风景秀丽。<br />驾车到三亚凤凰国际机场6.4公里；<br />驾车到三亚火车站9公里。
                    </div>



                    <div class="desc-info j-autohide" style="display:none">
                        公寓位于三亚湾景区，交通便利，附近有大型购物商城及便利店，购物极其便利；附近有各式餐厅，餐饮便利且丰富。
                    </div>

                    <div id="unitintromore" class="toggle-btn"><a href="javascript:;" class="">查看更多<i class="i-acor"></i></a></div>
                    <div id="unitintroless" class="toggle-btn" style="display:none"><a href="javascript:;" class="active">收起<i class="i-acor"></i></a></div>
                </div>
            </div>
            <script type="text/javascript">
                $("#unitintromore a").click(function () {
                    $("#unitintrocontentcontainer div.j-autohide").show();
                    $("#unitintromore").hide();
                    $("#unitintroless").show();
                });
                $("#unitintroless a").click(function () {
                    $("#unitintrocontentcontainer div.j-autohide").hide();
                    $("#unitintroless").hide();
                    $("#unitintromore").show();
                });

            </script>







            <!--小区 周边介绍 －开始（没有设置小区的自营房不显示周边介绍）-->
        </div>

        <!-- 入住须知  -->
        <div id="unitcheckinneedtoknow" class="m-column-box rule-cont">
            <!-- 入住规则  -->



            <div class="m-column-tit t-column-tit clearfix" id="unitIntro">
                <h2>入住须知</h2><span class="gray-text">（房屋编号：<b>90492</b>）</span>
            </div>
            <div class="column-info">
                <div class="side-box">
                    <i class="icon-hint"></i>
                    <h2>入住提示</h2>
                </div>
                <div class="content-box">
                    <div class="m-key-list t-key-list">
                        <ul>
                            <li class="">银联标识信用卡</li>
                            <li class="">14:00之后入住</li>
                            <li class="">12:00之前退房<i title='以上均为当地时间' class='icon-desc-text'></i></li>
                            <li class="">允许吸烟</li>
                            <li class="dis-text">提供发票</li>
                            <li class="dis-text">外卡信用卡</li>
                            <li class="dis-text">接待外宾</li>
                            <li class="dis-text">允许带宠物</li>
                        </ul>

                        <p>00:00~24:00接待</p>
                    </div>
                </div>
            </div>                <!-- 入住提示  -->


            <div class="column-info ">
                <div class="side-box">
                    <i class="icon-service"></i>
                    <h2>服务费用</h2>
                </div>
                <div class="content-box">
                    <div class="m-hint-info">
                        <ul class="desc-cont">
                            <li class="">
                                <h5>允许做饭</h5>
                            </li>
                            <li class="">
                                <h5>可加床</h5>
                                <div class="text-box">（最多可加床1张）</div>
                            </li>
                            <li class="">
                                <h5>打扫卫生</h5>
                                <div class="text-box">（1客1扫）</div>
                            </li>
                            <li class="">
                                <h5>被单更换</h5>
                                <div class="text-box">（1客1换）</div>
                            </li>
                            <li class="">
                                <h5>牙具更换</h5>
                                <div class="text-box">（1客1换）</div>
                            </li>
                            <li class="">
                                <h5>毛巾更换</h5>
                                <div class="text-box">（1客1换）</div>
                            </li>
                            <li class="dis-text">
                                <h5>允许加人</h5>
                            </li>
                            <li class="dis-text">
                                <h5>允许聚会</h5>
                            </li>
                        </ul>

                        <ul class="expense-cont">
                            <li>
                                <h5>其他入住须知</h5>
                                <div class="text-box">1、一人一证办理入住；
                                    2、公寓提供接站接机服务，具体收费情况请咨询前台；
                                    3、连住4晚及以上可提供厨具开火做饭；
                                    4、如需加床，收取加床费50元/张/晚。</div>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>

        </div>

        <!--地图－开始 -->
        <div id="unitmap" class="m-column-box">

            <div class="m-column-tit" id="unitmap">
                <h2>位置地图</h2>
            </div>
            <div class="map-cont">
                <div class="map-main">
                    <div id="mapPanel" class="map-panel"></div>
                    <div class="map-legend" id="mapIconInfo">
                        <div class="mapitem" lng="109.451134" lat="18.294809" address="三亚三亚湾海坡开发区中海路5号金中海蓝钻小区4号楼">
                            <div class="icon" id="icon1"></div>
                            <div class="text">房屋位置</div>
                        </div>
                    </div>

                </div>
                <div class="map-sidebar">
                    <div class="peripheral-info">
                        <h3>周边信息</h3>
                        <ul class="tab-menu" id="map-tab-menus">
                            <li class="current" q="餐饮"><a href="javascript:void(0)" onclick="_gaq.push(['_trackEvent', 'pcDetail', '地图','餐饮美食']);">餐饮美食</a></li>
                            <li q="景区"><a href="javascript:void(0)" onclick="_gaq.push(['_trackEvent', 'pcDetail', '地图','景区景点']);">景区景点</a></li>
                            <li q="娱乐" class="last"><a href="javascript:void(0)" onclick="_gaq.push(['_trackEvent', 'pcDetail', '地图','娱乐场所']);">娱乐场所</a></li>
                        </ul>
                        <div class="tab-info-item" id="map-tab-items">
                        </div>
                    </div>
                </div>

                <div class="hint-text">周边五百米服务：泳池</div>
            </div>
        </div>
        <!-- 房屋评论  -->


        <script type="text/javascript">
            var Tj = Tj || {};
            Tj.Detail = Tj.Detail || {};
            Tj.Detail.LoadUnitComment=1;
            Tj.Detail.IsGetAllComment=0;
            Tj.Detail.IsDistinctForUnit = 1;

        </script>


        <div id="commentsArea" class="m-column-box">
            <a name="comment" id="comment" href="#comment"></a>
            <div class="m-column-tit t-column-tit clearfix" id="commentsArea">
                <h2 id="commentsTitle">本房屋点评（<b>40</b>条）</h2>
                <div class="point-info" rel="j-experience-info">
                    发点评赢体验券，乐享体验房<i class="icon-desc-text"></i>
                </div>
            </div>
            <div class="cont-info">
                <div class="m-comment-cont" id="detailcommentwraper">
                    <div id="unitcommentsummary" class="comment-hd">
                        <div class="comprehensive-summary">
                            <div class="composite-score">
                                <span class="lab-text">综合评分</span>
                            <span class="big-star-box">
                                <i class="scores-info" style="width:100%"></i>
                            </span>
                                <span class="lab-text-vb"><b class="num">5 </b>分（满分5分）</span>
                            </div>
                            <div class="rating-term">
                                <span class="text-box first-box">整洁卫生<b class="h-text">5 </b>分</span>
                                <span class="text-box">交通位置<b class="h-text">5 </b>分</span>
                                <span class="text-box">管理服务<b class="h-text">5 </b>分</span>
                                <span class="text-box last-box">设施装修<b class="h-text">5 </b>分</span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="m-tips-wrap" id="j-experience-info" style="display: none;">
            <div class="tips-content experience-voucher">
                <h2>发点评赢体验券说明</h2>
                <div class="tips-info">
                    <ol>
                        <li>下单入住后发表精华点评（字数在100字符以上，并上传至少3张相关公寓图片），7个工作日内审核通过后，在第7个工作日，获得途家赠送的体验券。</li>
                        <li>金额在800元以下（含800元）的订单按金额1：1的比例赠送体验券，800元以上的订单一律赠送800元体验券。</li>
                    </ol>
                </div>
                <div class="line-box"></div>
                <h2 class="color-black ">使用说明</h2>
                <div class="tips-info">
                    <ol>
                        <li>体验券到帐后次日生效，有效期为120天。</li>
                        <li>预订时，“预订日期、入住日期和退房日期”都必须在体验券的有效期内。</li>
                        <li>预订时，如果体验券金额不足以抵扣全部房费，需线上支付差价。</li>
                        <li>体验房选自途家精选公寓，数量有限，旺季供不应求，先订先得。</li>
                        <li>同一用户（非业主会员），在同一家门店，180天（自然日）内最多可入住2天体验房。</li>
                        <li>业主会员预订体验房时，需录入入住人的真实身份信息。如果超出10个受益人范畴则不允许预订。</li>
                    </ol>
                </div>
            </div>
        </div>

        <!-- 达人体验  -->
    </div>

    <!--侧边栏-->
    <div class="sidebar-cont">
        <div class="m-column-box t-column-box">

            <div class="sidebar-hd sidebar-hd-new"><h2>房东信息</h2></div>
            <div class="m-listing-cont">
                <div class="hotel-info" id="hotelmanagementcompanycontent">
                    <div class="photo-cont">
                        <a href="/Hotel/Index/25587/" target="_blank" onclick="_gaq.push(['_trackEvent', 'pcDetail', '管理公司']);"><img src="http://pic.tujia.com/upload/LandlordStoreLogo/day_160329/201603291416041143.jpg" alt="" width="100" height="100" class="photo-img"></a>
                    </div>
                    <h2 class="name-box">
                        <a href="/Hotel/Index/25587/" target="_blank" class="listing-name" onclick="_gaq.push(['_trackEvent', 'pcDetail', '管理公司']);">三亚瑞家海景套房公寓（金中海蓝钻店） </a><span class="personal-tag landlord-tag" title="个人房屋分享，最有家的感觉">个人房东</span>
                        <a href="http://bp.tujia.com/2016/SelectMerchants" target="_blank" class="icon-quality-hotel" rel="PreferredUnitTips"></a>
                    </h2>
                    <div class="shop-tag">
                        <span class="lab-box"><i class="icon-real"></i>实名认证</span>
                    </div>

                    <div class="shop-rate">

                        <p class="link-btn-cont"><a ownerid="" ownername="三亚瑞家海景套房公寓（金中海蓝钻店）" unitid="90492" hotelid="25587" data="2" href="javascript:;" class="chat-btn custom-service" onclick="_gaq.push(['_trackEvent', 'pcDetail', '和房东聊天']);">和房东聊天</a></p>
                        <p class="contact-info">
                            联系电话：<span class="tel-box">400-606-1230 转 34838</span>
                            <br /><span class="gray-text">(7x24小时)</span>
                        </p>
                    </div>
                </div>


                <div class="sidebar-hd"><h2>房东的其它房屋<span class="gray-text">（4）</span></h2></div>
                <div class="m-house-cont">
                    <div class="houses-list">
                        <ul>
                            <li>
                                <a href="/sanya_gongyu/se0/sanyawan_90463.htm" target="_blank" class="pic-cont" onclick="_gaq.push(['_trackEvent', 'pcDetail', '房东其他房屋']);"><img src="http://pic.tujia.com/upload/landlordunit/day_160328/thumb/201603281253275966_300_200.jpg" alt="" width="278" height="165"></a>
                                <div class="house-cont">
                                    <a href="/sanya_gongyu/se0/sanyawan_90463.htm" target="_blank" class="tit" onclick="_gaq.push(['_trackEvent', 'pcDetail', '房东其他房屋']);">三亚瑞家公寓揽海精品海景大床房</a>

                                    <div class="house-info">
                                        <span class="lab-text" title="普通公寓">普通公寓</span>|
                                        <span class="lab-text" title="1室1卫">1室1卫</span>|
                                        <span class="lab-text" title="1张床">1张床</span>|
                                        <span class="lab-text" title="宜住2人">宜住2人</span>
                                    </div>
                                    <div class="picer-box"><dfn>¥</dfn><span class="value-box">148</span>/晚</div>
                                </div>
                            </li>
                            <li>
                                <a href="/sanya_gongyu/se0/sanyawan_90502.htm" target="_blank" class="pic-cont" onclick="_gaq.push(['_trackEvent', 'pcDetail', '房东其他房屋']);"><img src="http://pic.tujia.com/upload/landlordunit/day_160422/thumb/201604220921471859_300_200.jpg" alt="" width="278" height="165"></a>
                                <div class="house-cont">
                                    <a href="/sanya_gongyu/se0/sanyawan_90502.htm" target="_blank" class="tit" onclick="_gaq.push(['_trackEvent', 'pcDetail', '房东其他房屋']);">三亚瑞家公寓揽海精品双标房</a>

                                    <div class="house-info">
                                        <span class="lab-text" title="普通公寓">普通公寓</span>|
                                        <span class="lab-text" title="1室1卫">1室1卫</span>|
                                        <span class="lab-text" title="2张床">2张床</span>|
                                        <span class="lab-text" title="宜住2人">宜住2人</span>
                                    </div>
                                    <div class="picer-box"><dfn>¥</dfn><span class="value-box">188</span>/晚</div>
                                </div>
                            </li>
                            <li>
                                <a href="/sanya_gongyu/se0/sanyawan_102431.htm" target="_blank" class="pic-cont" onclick="_gaq.push(['_trackEvent', 'pcDetail', '房东其他房屋']);"><img src="http://pic.tujia.com/upload/landlordunit/day_160506/thumb/201605061018476628_300_200.jpg" alt="" width="278" height="165"></a>
                                <div class="house-cont">
                                    <a href="/sanya_gongyu/se0/sanyawan_102431.htm" target="_blank" class="tit" onclick="_gaq.push(['_trackEvent', 'pcDetail', '房东其他房屋']);">三亚瑞家公寓揽海精品一房一厅</a>

                                    <div class="house-info">
                                        <span class="lab-text" title="普通公寓">普通公寓</span>|
                                        <span class="lab-text" title="1室1厅1卫">1室1厅1卫</span>|
                                        <span class="lab-text" title="2张床">2张床</span>|
                                        <span class="lab-text" title="宜住4人">宜住4人</span>
                                    </div>
                                    <div class="picer-box"><dfn>¥</dfn><span class="value-box">208</span>/晚</div>
                                </div>
                            </li>
                        </ul>
                        <div class="link-btn-cont"><a href="/Hotel/Index/25587/" target="_blank" onclick="_gaq.push(['_trackEvent', 'pcDetail', '查看更多']);">查看更多&gt;&gt;</a></div>
                    </div>
                </div>
            </div>

        </div>
        <!--管理公司其它房屋-->
        <div class="m-column-box t-column-box">

        </div>

        <div class="m-column-box ">
            <div class="m-ensure">
                <i class="icon-ensure"></i>
                <div class="text-cont">
                    <h4>1000万<a href="http://www.tujia.com/Promotion/Tujiabaozhangjihua.htm" target="_blank" class="link-btn" onclick="_gaq.push(['_trackEvent', 'pcDetail', '安心租100%保障']);">安心租100%保障</a></h4>
                    <p>房屋描述失真、到店无房、房东加价全部退订金并补偿</p>
                </div>
            </div>
        </div>
    </div>
</div>
<form action="/UnitDetail/Log/" data-ajax="true" data-ajax-method="Post" id="logForm" method="post"><input id="unitId" name="unitId" type="hidden" value="90492" /></form>    <div class="m-tips-wrap" id="j-experience-info" style="display: none;">
    <div class="tips-content experience-voucher">
        <h2>发点评赢体验券说明</h2>
        <div class="tips-info">
            <ol>
                <li>下单入住后发表精华点评（字数在100字符以上，并上传至少3张相关公寓图片），7个工作日内审核通过后，在第7个工作日，获得途家赠送的体验券。</li>
                <li>金额在800元以下（含800元）的订单按金额1：1的比例赠送体验券，800元以上的订单一律赠送800元体验券。</li>
            </ol>
        </div>
        <div class="line-box"></div>
        <h2 class="color-black ">体验券使用说明</h2>
        <div class="tips-info">
            <ol>
                <li>体验券到帐后次日生效，有效期为120天。</li>
                <li>预订时，“预订日期、入住日期和退房日期”都必须在体验券的有效期内。</li>
                <li>预订时，如果体验券金额不足以抵扣全部房费，需线上支付差价。</li>
                <li>体验房选自途家精选公寓，数量有限，旺季供不应求，先订先得。</li>
                <li>同一用户（非业主会员），在同一家门店，180天（自然日）内最多可入住2天体验房。</li>
                <li>业主会员预订体验房时，需录入入住人的真实身份信息。如果超出10个受益人范畴则不允许预订。</li>
            </ol>
        </div>
    </div>
</div>
<div id="PreferredUnitTips" class="m-tips-wrap" style="display: none">
    <div class="tips-content" style="width:220px">
        <div class="tips-info">途家团队实地验真，设施可靠，服务优质。途家团队贴心保障，您的出行入住首选。</div>
    </div>
</div>
<!-- 结束   2015-10-19  -->





<!--跳转弹层－开始-->


<div id="divGoTujia" class="pop-wrap" style="display: none; z-index: 10000">
    <div class="p-content">
        <div class="p-text">
        </div>
    </div>
</div>
<!--跳转弹层－结束-->
<!--在线客服&& 和房东聊天－开始-->
<div class="J-service-btn" id="J-service-btn">
    <a href="javascript:;" rel="nofollow" onclick="qimoChatClick(); return false;" class="service-chat-btn"><i class="icon-service"></i><span class="label-text">客服</span></a>
</div>
<!--在线客服&& 和房东聊天-结束-->
<!--登录框－开始-->
<div style="height: 1px;" id="loginBtn"></div>
<div class="m-login-wrap " id="userPop">
    <div class="login-head">

        <h2>登录即享：积分余额抵订金，高级会员有优惠</h2>
        <i class="close-btn  closeSubmit" title="关闭"></i>
    </div>
    <div class="login-tabs">
        <a id="tabSmsLogin" href="javascript:;" class="phone-pwd">短信快捷登录/注册</a>
        <a id="tabPwdLogin" href="javascript:;" class="user-pwd">普通登录</a>
    </div>
    <div class="login-body">
        <div id="tabPwdLoginContent" class="dn">
            <div class="column-box">
                <p>注册立送<span class="highlight-box">100</span> 元礼品卡，预订立即可用</p>
            </div>

            <div class="control-group" style="margin-top:10px;">
                <div class="controls">
                    <label for="account">
                        <input type="text" class="ipt-text" name="username" id="account" value="" />
                        <span class="text-watermark">用户名/手机号/邮箱</span>
                    </label>
                </div>
            </div>

            <div class="control-group">
                <div class="controls">
                    <label for="password">
                        <input type="password" class="ipt-text" name="pwd" id="password" />
                        <span class=" text-watermark">密码</span>
                    </label>
                </div>
                <div class="validationError dn" id="loginerrormsg"></div>
            </div>

            <div class="control-group  " id="validateArea">
                <div class="controls">
                    <label for="verifycode">
                        <input type="text" class="ipt-text verify-ipt" name="verifycode" id="verifycode" />
                        <span class="text-watermark" style="width: 140px;">请输入计算结果</span>
                        <img id="verifyImage" alt="看不清楚，换一张" class="verify-img" />
                        <a id="verifyImageLink" href="javascript:void(0)" class="change-rand">看不清，换一张</a>
                    </label>
                </div>
                <div class="validationError dn" id="verifycodeErorr"></div>
            </div>

            <div class="control-group t-control-group">
                <div class="controls">

                    <label for="saveUserLongTime"><input type="checkbox" name="" class="check-btn " id="saveUserLongTime">保存登录状态2周</label>

                    <a href="https://passport.tujia.com/PortalSite/FetchPassword/" class="forget-pwd" target="_blank">忘记密码？</a>

                    <input type="hidden" name="userExpiresHours" id="userExpiresHours" value="" />
                </div>
            </div>

            <div class="control-group btn-group">
                <div class="controls">
                    <input type="button" value="登录" class="login-btn" id="loginSubmit" style="cursor:pointer;" />
                    <input type="button" value="登录中" class="login-btn" style="display:none;" id="loginSubmitIng" />

                </div>

            </div>

            <div class="coagent-cont">
                <div class="coagent-tit"><span>合作网站账号登录</span></div>
                <div class="link-btn-cont">
                    <a href="javascript:;" id="qqLogin" title="QQ账户登录" onclick="qqLogin();return false;"  class="qq-login-btn">QQ</a>
                    <a href="javascript:;" id="sinaLogin" title="新浪微博账户登录" onclick="sinaLogin();return false;" class="sian-login-btn">新浪微博</a>
                    <a href="javascript:;" id="" title="微信账户登录" onclick="wechatLogin();return false;" class="weixin-login-btn">微信</a>
                </div>
            </div>
            <div class="link-group j-logon-directorder"><a href="" target="_blank">不登录，直接预订 >></a></div>


            <script type="text/javascript">
                $('#loginSubmit').bind('click', function () {
                    if (!$("#account").val() || !$("#password").val()) {
                        $('#loginerrormsg').html("用户名或密码不能为空！").show();
                        return;
                    }

                    $("#loginSubmit").hide();
                    $("#loginSubmitIng").show();
                    $.ajax({
                        url: 'https://passport.tujia.com/PortalSite/AjaxLogin/',
                        data: {
                            'username': $('#account').val()
                            , 'pwd': $('#password').val()
                            , 'verifycode': $('#verifycode').val()
                            , 'userExpiresHours': $('#userExpiresHours').val()
                            , 'currentUrl': window.location.href
                        },
                        type: 'post',
                        dataType: 'jsonp',
                        success: function (res) {
                            if (res.IsSuccess) {
                                if (res.RedirectUrl) {
                                    window.location.href = res.RedirectUrl;
                                    return;
                                }

                                if ($("#showButton").val() == "3" || $("#showButton").val() == "4") {
                                    if (window.location.href.indexOf("?") > -1) {
                                        window.location.href = window.location.href + "&show=" + $("#showButton").val();
                                    }
                                    else {
                                        window.location.href = window.location.href + "?show=" + $("#showButton").val();
                                    }
                                }
                                else {
                                    var redirectUrl = '';

                                    var href = redirectUrl == "" ? $("div.j-logon-directorder > a").attr("href") : redirectUrl;
                                    if (href) {
                                        window.location.href = href;
                                    }
                                    else {
                                        window.location.reload();
                                    }
                                }

                            } else {
                                $("#loginSubmit").show();
                                $("#loginSubmitIng").hide();
                                if (res.ErrorCount >= 5) {
                                    $('#validateArea').show();
                                }
                                if (res.ErrorCode == 1) {
                                    $('#loginerrormsg').html(res.Message).show();
                                    $('#verifycodeErorr').hide();
                                } else {
                                    $('#validateArea').show();
                                    $('#loginerrormsg').hide();
                                    $('#verifycodeErorr').html(res.Message).show();
                                }
                                $('#verifyImageLink').click();
                            }
                        }
                    });
                });

                $('#account,#password,#verifycode').focus(function () {
                    $(this).next().hide();

                    $('#loginerrormsg').hide();
                    $('#verifycodeErorr').hide();
                }).blur(function () {
                    if ($.trim($(this).val()) == "") {
                        $(this).next().show();
                    }
                });

                $("#saveUserLongTime").click(function () {
                    if (this.checked) {
                        $("#userExpiresHours").val(350);
                    } else {
                        $("#userExpiresHours").val("");
                    }
                });

                $("#verifyImage,#verifyImageLink").click(function () {
                    var verifyimagesrc = 'https://passport.tujia.com/PortalSite/VerifyImage/?r=' + Math.random();
                    $("#verifyImage").attr("src", verifyimagesrc);
                    return false;
                });

                function qqLogin() {
                    window.location = "https://passport.tujia.com/PortalSite/QQLogin?srcUrl=http%3a%2f%2fwww.tujia.com%2fsanya_gongyu%2fse0%2fsanyawan_90492.htm";
                }

                function sinaLogin() {
                    window.location = "https://passport.tujia.com/PortalSite/SinaLogin?srcUrl=http%3a%2f%2fwww.tujia.com%2fsanya_gongyu%2fse0%2fsanyawan_90492.htm";
                }

                function wechatLogin() {
                    window.location = "https://passport.tujia.com/PortalSite/WeChatLogin?srcUrl=http%3a%2f%2fwww.tujia.com%2fsanya_gongyu%2fse0%2fsanyawan_90492.htm";
                }
            </script>

        </div>
        <div id="tabSmsLoginContent" class="dn">

            <div class="column-box">
                <p>注册立送<span class="highlight-box">100</span> 元礼品卡，预订立即可用</p>

            </div>
            <div class="control-group" style="margin-top:10px;">
                <div class="controls">
                    <label for="smobile">
                        <input type="text" class="ipt-text" name="smobile" id="smobile" value=""/>
                        <span class="text-watermark">手机号</span>
                    </label>
                </div>
            </div>

            <div class="control-group  " id="svalidatearea">
                <div class="controls">
                    <label for="sverifycode">
                        <input type="text" class="ipt-text verify-ipt" name="sverifycode" id="sverifycode" />
                        <span class="text-watermark" style="width: 140px;">请输入计算结果</span>
                        <img id="sverifyImage" alt="看不清楚，换一张" class="verify-img" />
                        <a id="sverifyImageLink" href="javascript:void(0)" class="change-rand">看不清，换一张</a>
                    </label>
                </div>
            </div>

            <div class="control-group  pwd-group">
                <div class="controls">
                    <label for="spassword">
                        <input type="text" class="ipt-text" name="code" id="spassword" />
                        <span class=" text-watermark">动态密码</span>
                    </label>

                    <input type="button" id="sbtnGetVerifyCode" value="获取手机验证码" class="verify-btn" />
                </div>
                <div class="validationError dn" id="sloginerrormsg"></div>
            </div>




            <div class="control-group btn-group">
                <div class="controls">
                    <input type="button" value="登录/注册" class="login-btn" id="sloginSubmit" style="cursor:pointer;" />
                    <input type="button" value="登录/注册中" class="login-btn" style="display:none;" id="sloginSubmitIng" />

                </div>

            </div>



            <div class="coagent-cont">
                <div class="coagent-tit"><span>合作网站账号登录</span></div>
                <div class="link-btn-cont">
                    <a href="javascript:;" id="qqLogin" title="QQ账户登录" onclick="qqLogin();return false;"  class="qq-login-btn">QQ</a>
                    <a href="javascript:;" id="sinaLogin" title="新浪微博账户登录" onclick="sinaLogin();return false;" class="sian-login-btn">新浪微博</a>
                    <a href="javascript:;" id="" title="微信账户登录" onclick="wechatLogin();return false;" class="weixin-login-btn">微信</a>
                </div>
            </div>
            <div class="link-group j-logon-directorder"><a href="" target="_blank">不登录，直接预订 >></a></div>


            <script type="text/javascript">

                $('#sloginSubmit').click(function () {
                    if (!$("#smobile").val() || !$("#spassword").val()) {
                        $('#sloginerrormsg').html("电话或短信验证码不能为空！").show();
                        return;
                    }

                    $("#sloginSubmit").hide();
                    $("#sloginSubmitIng").show();


                    $.ajax({
                        url: 'https://passport.tujia.com/PortalSite/SmsAjaxLogin/',
                        data: {
                            'smobile': $('#smobile').val(), 'spassword': $('#spassword').val(), 'currentUrl': window.location.href
                        },
                        type: 'post',
                        dataType: 'jsonp',
                        success: function (res) {
                            if (res.IsSuccess) {

                                if (res.RedirectUrl) {
                                    window.location.href = res.RedirectUrl;
                                    return;
                                }

                                var redirectUrl = '';

                                var href = redirectUrl == "" ? $("div.j-logon-directorder > a").attr("href") : redirectUrl;
                                if (href) {
                                    window.location.href = href;
                                }
                                else {
                                    window.location.reload();
                                }
                            } else {
                                $("#sloginSubmit").show();
                                $("#sloginSubmitIng").hide();
                                $('#sloginerrormsg').html(res.Message).show();
                            }
                        }
                    });

                });

                var countdownTimeWait = 60;
                function CountdownTime() {

                    if (countdownTimeWait == 0) {
                        $('#sbtnGetVerifyCode').removeAttr("disabled").val('获取动态密码');
                        countdownTimeWait = 60;
                    } else {
                        $('#sbtnGetVerifyCode').attr("disabled", true).val('重新发送(' + countdownTimeWait + ')');
                        countdownTimeWait--;
                        setTimeout(function () {
                            CountdownTime();
                        }, 1000)
                    }
                }

                $('#sbtnGetVerifyCode').click(function () {
                    var $sverifycode = $("#sverifycode");

                    if (!$("#svalidatearea").hasClass("dn") && !$sverifycode.val()) {
                        $('#sloginerrormsg').html("验证码不能为空！").show();
                        return;
                    }

                    var $smobile = $("#smobile");

                    if (!$smobile.val()) {
                        $('#sloginerrormsg').html("手机号不能为空！").show();
                        return;
                    }

                    var re = /^1\d{10}$/;
                    if (!re.test($smobile.val())) {
                        $('#sloginerrormsg').html("手机号不正确！").show();
                        return;
                    }

                    $('#sloginerrormsg').html("正在发送验证码至你的手机，请稍等！").show();

                    $.ajax({
                        url: 'https://passport.tujia.com/PortalSite/SmsSendVerifyCode/',
                        data: {
                            'smobile': $smobile.val(), 'sverifycode': $sverifycode.val()
                        },
                        type: 'post',
                        dataType: 'jsonp',
                        success: function (res) {

                            $('#sloginerrormsg').html(res.Message).show();

                            if (!res.IsSuccess) {
                                $('#sverifyImageLink').click();
                            }
                            else {
                                CountdownTime();
                            }
                        }
                    });
                });




                $('#smobile,#spassword,#sverifycode').focus(function () {
                    $(this).next().hide();

                    $('#sloginerrormsg').hide();
                }).blur(function () {
                    if ($.trim($(this).val()) == "") {
                        $(this).next().show();
                    }
                });

                $("#saveUserLongTime").click(function () {
                    if (this.checked) {
                        $("#userExpiresHours").val(350);
                    } else {
                        $("#userExpiresHours").val("");
                    }
                });

                $("#sverifyImage,#sverifyImageLink").click(function () {
                    var sverifyImagesrc = 'https://passport.tujia.com/PortalSite/VerifyImage/?r=' + Math.random();
                    $("#sverifyImage").attr("src", sverifyImagesrc);
                    return false;
                });


            </script>

        </div>
    </div>
</div>
<script type="text/javascript">

    $("#loginBtn").overlay({
        top: "center",
        mask: {
            color: '#000',
            loadSpeed: 100,
            opacity: 0.5
        },
        target: "#userPop",
        closeOnClick: false,
        close: ".closeSubmit,div.link-group > a",
        fixed: false,
        onBeforeLoad: function () {
            $(".login-tabs").children().first().click();
        }

    });

    $("#tabPwdLogin").click(function () {
        $("#tabPwdLogin").addClass("isActive");
        $("#tabSmsLogin").removeClass("isActive");

        $("#tabPwdLoginContent").show();
        $("#tabSmsLoginContent").hide();
        $("#verifyImageLink").click();

        if ($("#account").val() !== "")
            $("#account").focus();
    });

    $("#tabSmsLogin").click(function () {
        $("#tabSmsLogin").addClass("isActive");
        $("#tabPwdLogin").removeClass("isActive");

        $("#tabPwdLoginContent").hide();
        $("#tabSmsLoginContent").show();
        $("#sverifyImageLink").click();

        if ($("#smobile").val() !== "")
            $("#smobile").focus();
    });

    $("#tabPwdLoginContent,#tabSmsLoginContent").on("keydown", "input:visible", function (event) {
        if (event.keyCode != '13') {
            return;
        }

        $(event.delegateTarget).find("input.login-btn:button:visible").click();
        return false;
    })
</script>
<!--登录框－结束-->




<!-- footer Start -->







<div class="m-dld-wrap">
    <div class="dld-bd">
        <div class="game-info" style="width: 480px;">
            <span style="display: block; width: 452px;height: 172px;margin: -22px 0 0 0;background: url(http://staticfile.tujia.com/PortalSite2/Images/girl.png) no-repeat top;"></span>
        </div>

        <div class="dld-ct">
            <div class="dld-ct-item">
                <h2>扫描二维码下载</h2>
                <i class="i-code-app"></i>
            </div>
            <div class="dld-ct-item">
                <h2>直接下载</h2>
                <a onclick="window.open('http://m.tujia.com/click?id=1184');return false;" href="javascript:void(0);" class="link-btn" target="_blank">iPhone</a>
                <a onclick="window.open('http://download.tujia.com/android/tujia4541-tujiaportal.apk'); return false;" href="javascript:void(0);" class="link-btn" target="_blank">Android</a>
            </div>
        </div>

        <div class="weixin-ct">
            <h2> 实时热门推荐，请关注途家微信</h2>
            <i class="i-code-weixin"></i>
        </div>

        <a href="javascript:void(0)" class="close-btn" title="关闭">关闭</a>
    </div>
</div>


<div class="m-seo-list">



</div>

<div style="display: none">getFriendLinkRuleTime执行时间: 0.1155</div>
<div style="display: none">getSeoFooterRulrTime执行时间: 0.0198</div>
<div style="display: none">SeoFooter执行时间: 0.0044</div>




<div class="m-footer-link-list">
    <a href="http://content.tujia.com/tujiajianjie.htm" target="_blank" class="forst" rel="nofollow">关于我们</a>|
    <a href="http://content.tujia.com/youkebangzhu.htm" target="_blank" rel="nofollow">我是房客</a>|
    <a href="http://content.tujia.com/qiyewenhua.htm" target="_blank" rel="nofollow">加入途家</a>|
    <a href="/SiteMap/UnitDestination/" target="_blank">网站地图</a>|
    <a href="/sitemap.htm" target="_blank">城市地图</a>




</div>


<!-- footer  End -->
<div class="edm-pop " id="dialog" style="display: none">
    <div class="edm-hd"><h2>订阅邮件</h2><a class="close-btn" title="关闭"></a></div>
    <div class="edm-bd">
        <div class="msg-box">
            <img src="http://staticfile.tujia.com/portalsite2/images/loading.gif"/>正在提交订阅
        </div>
    </div>
</div>
<script type="text/javascript">
    var _mvq = _mvq || [];
    _mvq.push(['$setAccount', 'm-21039-1']);
    _mvq.push(['$logConversion']);
    (function () {
        var mvl = document.createElement('script');
        mvl.type = 'text/javascript';
        mvl.async = true;
        mvl.src = 'https://material-ssl.mediav.com/bjjs/mba_ka.js';
        var s = document.getElementsByTagName('script')[0];
        s.parentNode.insertBefore(mvl, s);
    })();
</script>



<script type="text/javascript">
    setTimeout(function(){var a=document.createElement("script");
        var b=document.getElementsByTagName("script")[0];
        a.src=document.location.protocol+"//script.crazyegg.com/pages/scripts/0025/8248.js?"+Math.floor(new Date().getTime()/3600000);
        a.async=true;a.type="text/javascript";b.parentNode.insertBefore(a,b)}, 1);
</script>


<script type="text/javascript">
    _mvq.push(['$setGeneral', 'goodsdetail', '', /*用户名*/ '', /*用户id*/ '4312917']);
    _mvq.push(['$logConversion']);
    _mvq.push(['$addGoods', '', '', '', '90492', '', '', '', '', '', '', '', '']);
    _mvq.push(['$logData']);
</script>



<script>
    (function () {

        var bp = document.createElement('script');
        bp.async = 'async';
        bp.src = '//push.zhanzhang.baidu.com/push.js';
        var s = document.getElementsByTagName("script")[0];
        s.parentNode.insertBefore(bp, s);
    })();
</script>

<script type="text/javascript" id="bdshare_js" data="type=tools&amp;mini=1&amp;uid=671531"></script>
<script type="text/javascript">
    //var subtype = ViewBag.subtype == null ? UnitDetailSubType.None : (UnitDetailSubType)ViewBag.subtype;
    var helpUrl = "http://content.tujia.com/help/index.htm#tv";

    var initDestinationPinyin = "sanya";

    var traceData = window.traceData || {};
    traceData.unitId = 90492;
</script>
<script type="text/javascript" src="http://staticfile.tujia.com/portalsite2/scripts/js_detail_foot_v3?v=405bfef"></script>
<link rel="stylesheet" href="http://staticfile.tujia.com/Common/tooltip/poshytip/tip-white/tip-white.css" type="text/css" />

<script type="text/javascript">

    function getProductId() {
        var fullUrl = location.href;
        var pid = 0;
        if (fullUrl.indexOf("#") > -1) {
            var params = fullUrl.substring(fullUrl.indexOf("#") + 1, fullUrl.length);
            var paramsArray = params.split("&");
            for (var i = 0; i < paramsArray.length; i++) {
                if (paramsArray[i].indexOf("pid") > -1) {
                    if (paramsArray[i].split("=").length == 2) {
                        pid = paramsArray[i].split("=")[1];
                    }
                }
            }

        }
        return pid;
    }

    function AddBestProductEvents() {

        $(".j-product-announcement").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });
        $(".j-bestproduct-experience, .point-info").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });
        $("#booknow").click(function (event) {
            event.preventDefault();
            var self = $(this);
            if (self.attr("href").indexOf("#") == 0) {
                $.scrollTo($(self.attr('href')).offset().top - 38, 600, function () {
                });
            }
        });

        $("#showallproducts").click(function () {
            var self = $(this);
            self.addClass("dn");
            $(".autohide").removeClass("dn");
            $("#showallproducts2").removeClass("dn");
        });

        $("#showallproducts2").click(function () {
            var self = $(this);
            self.addClass("dn");
            $(".autohide").addClass("dn");
            $("#showallproducts").removeClass("dn");
        });


        // 服务tips
        $(".j-productservice").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });


        // 礼包tips事件
        $(".j-detail-gift").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });
        // 免费接机Tips事件
        $(".j-detail-freepickupair").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });
        // 免费接站Tips事件
        $(".j-detail-freepickuptrain").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });
        // 免费早餐Tips事件
        $(".j-detail-freebreakfast").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });
        // 预订限制Tips事件
        $(".j-detail-multilimit").powerFloat({
            eventType: "hover",
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });

        $("#bestProductreturn").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });
        $("#bestProductrefund").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });

        $("#bestProductvouchers").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });

        $("#unitownerreturncash").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });

        $(".j-product-changerule").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });


        $(".j-product-refund").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });

        $(".j-product-return").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });

        $(".j-detail-packgift").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });

        $(".m-ad-text span").eq(0).powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });

        $("#supportfullprepayicon,#supportfullprepayicon2").powerFloat({
            container: "plugin",
            reverseSharp: true,
            position: "7-5"
        });
    }


    if (Tj.Detail.LoadProduct) {
        function refreshProduct(isInit){
            $.ajax({
                url: getProductUrl,
                data: {
                    unitId: $("#UnitId").val(),
                    productId: getProductId(),
                    unitDetailSubType: Tj.Detail.Subtype,
                    startTime: $("#unitDetailStartDate").val(),
                    endTime: $("#unitDetailEndDate").val(),
                    bookingCount: 1,
                    prevId: (traceData && traceData.pageId) ? traceData.pageId : ''
                },
                type: "POST",
                beforeSend: function () { $("#productcontent").html('<div class="loading-wrap"><i class="icon-loading"></i></div>'); $("#bestProduct").html('<div class="loading-wrap"></div>'); },
                cache: false,
                dataType: "html",
                complete: function () { if(isInit) initScrollto(); },
                success: function (retData) {
                    var res = $(retData);
                    $("#productcontent").html(retData);
                    if (res.attr("data-spring") !== undefined) {
                        $(".h-tag").show();
                    }
                    $("#bestProduct").html($("#economicalProduct").html());
                    $("#pricebox").html($("#economicalProductPrice").html());
                    AddBestProductEvents();
                },
                error: function () {
                    $("#productcontent").html('<div class="no-products" data-error="true" >在你选择的日期内没有可用产品</div>');
                    $("#bestProduct").html('<div class="no-room-tips">此时段该管理公司不接受预订</div>');
                }
            });
        }

        $("#changeBtn").click(function () {
            refreshProduct();
        });
        refreshProduct(true);
    }

    $(document).delegate(".detail-btn", "click", null, function () {
        var allcomments = $(this).siblings("span.j-unitcomment-allcomments");
        if (allcomments.is(":visible")) {
            $(this).siblings("span.j-unitcomment-ellipsis").show();
            $(this).siblings("span.j-unitcomment-allcomments").hide();
            $(this).html("展开全部");
        } else {
            $(this).siblings("span.j-unitcomment-ellipsis").hide();
            $(this).siblings("span.j-unitcomment-allcomments").show();
            $(this).html("收起全部");
        }
    });

    var unitCalendarUrl = '/UnitDetail/GetUnitCalendar?unitId=90492&subType=None&beginDate=2016-05-19';
    /*create default house status data*/
    (function () {
        var now = new Date();
        var houseData = [];
        var date;
        var monthData = [];
        for (var i = 1; i <= 12; i++) {
            monthData = [];
            date = new Date(now.getFullYear(), now.getMonth() + i, 0);
            for (var j = 0; j < date.getDate() ; j++) {
                monthData.push([1]);
            }
            houseData.push(monthData);
        }
        window.dateHouseStatus = houseData;
    })();
</script>

<script type="text/javascript" src="http://api.map.baidu.com/api?v=2.0&ak=RquoEX471C87SGjM3kAkDLPX"></script>

<script src="http://staticfile.tujia.com/PortalSite2/Scripts/Base/common-date.js" type="text/javascript"></script>

<script type="text/javascript">
    if (Tj.Detail.LoadMap) {
        if (mapType == 2) {
            Tujia.DetailMapGoogle().Init();
        } else {
            Tujia.DetailMapBaidu().Init();
        }
    }
</script>
<!-- 携程追踪代码 -->


<script type="text/javascript" src="http://staticfile.tujia.com/portalsite2/scripts/RongIMLib-2.1.0.min.js?v=405bfef"></script><script type="text/javascript" src="http://staticfile.tujia.com/portalsite2/scripts/im_v2.js?v=405bfef"></script>        <script type='text/javascript' src='http://webchat.7moor.com/javascripts/7moorInit.js?accessId=797098a0-b29d-11e5-b3b1-49764155fe50&autoShow=false' async='async'></script>


<style>
    .black_overlay {
        display: none;
        position: absolute;
        top: 0%;
        left: 0%;
        width: 100%;
        height: 200%;
        background-color: black;
        z-index: 1001;
        -moz-opacity: 0.8;
        opacity: .80;
        filter: alpha(opacity=80);
    }

    .white_content {
        display: none;
        position: absolute;
        top: 25%;
        left: 35%;
        width: 25%;
        /*height: 45%;
            padding: 20px;*/
        border: 1px solid #ccc;
        background-color: white;
        z-index: 1002;
        overflow: auto;
    }

    .white_content li {
        font-size: 12px;
    }
</style>

<div id="light1" class="m-elastic-layer white_content">

</div>
<!--弹出层时背景层DIV-->
<div id="divfadebg" class="black_overlay"></div>
<script type="text/javascript">

    (function () {
        //alert('bg');
        //var CustomerInfoEnumCustomerLevel = $("#CustomerInfoEnumCustomerLevel").val();
        //alert(CustomerInfoEnumCustomerLevel);
        var highLevelCustomerNoticeStatus = $.cookie("highLevelCustomerNoticeStatus");
        if (highLevelCustomerNoticeStatus === null) {
            //alert('null1');

            setTimeout(function () {
                getEnumCustomerLevel();
            }, 1000);





        } else {
            //alert(highLevelCustomerNoticeStatus);
            //alert('already-show');
        }


    })();


    function getEnumCustomerLevel() {
        $.ajax({
            type: "Get",
            url: "/Common/GetEnumCustomerLevel/t=" + new Date().getTime(),
            success: function (data) {

                //if (data == "false") {
                //    //alert('nohighLevel');
                //} else {
                //    $.cookie("highLevelCustomerNoticeStatus", 1, { expires: 100 });
                //    $(".highcustomerlevel-text").html(data);
                //    if (data == "业主" || data == "钻石会员" || data == "白金会员" || data == "途家员工") {
                //        $(".maxrate-text").html("5%-10");
                //    } else {
                //        $(".maxrate-text").html("3%-5");
                //    }
                //    showDialog("light1", "divfadebg", 0);
                //}

                if (!data) {
                    return;
                }

                $("#light1").empty().html(data);
                showDialog("light1", "divfadebg", 0);
            }
        });
    }

    function showDialog(id, bg_div, ticketCount) {
        var $dialog = $("#"+id);
        $(".black_overlay").height(document.body.clientHeight);
        $(".black_overlay").width(document.body.clientWidth);
        document.getElementById(id).style.display = 'block';

        document.getElementById(bg_div).style.display = 'block';

        var topy = 300 + ticketCount * 30;
        document.getElementById(id).style.top = topy + "px";
        $dialog.css({
            left: (document.body.clientWidth - $dialog.width()) / 2
        });
    }
    function closeBtn(id, bg_div) {
        $.cookie("highLevelCustomerNoticeStatus", 1, { expires: 100 });
        document.getElementById(id).style.display = 'none';
        document.getElementById(bg_div).style.display = 'none';
    }
</script>







<script src="http://content.tujia.com/TuJiaPrefAnalytics.js" type="text/javascript"></script>
<script>
    tuJiaPrefAnalytics.post({ site: 'PortalSite', pageName: 'UnitDetail_Index' });
</script>

<!--底部 Google analytics访问分析代码/ 百度统计 -->
<script type="text/javascript">
    var _gaq = _gaq || [];
    var _bdhmProtocol = (("https:" == document.location.protocol) ? " https://" : " http://");
    _gaq.push(['_setAccount', 'UA-30952525-1']);
    _gaq.push(['_setDomainName', 'tujia.com']);
    //_gaq.push(['_setAllowHash', false]);
    _gaq.push(['_addIgnoredRef', 'tujia.com']);
    _gaq.push(['_addOrganic', 'soso', 'w']);
    _gaq.push(['_addOrganic', 'youdao', 'q']);
    _gaq.push(['_addOrganic', 'sogou', 'query']);
    _gaq.push(['_addOrganic', 'baidu', 'wd']);
    _gaq.push(['_addOrganic', 'baidu', 'word']);
    _gaq.push(['_addOrganic', 'bing', 'q']);
    _gaq.push(['_addOrganic', '360', 'q']);
    _gaq.push(['_addOrganic', '360', 'wd']);
    _gaq.push(['_addOrganic', 'so.com', 'q']);
    _gaq.push(['_addOrganic', 'so.com', 'wd']);
    if ($("#btnDianzan").length > 0) {
        $("#btnDianzan").click(function () {
            if ($(this).hasClass("active")) {
                _gaq.push(['_trackEvent', 'pcDetail', '取消点赞']);
            } else {
                _gaq.push(['_trackEvent', 'pcDetail', '点赞']);
            }
        });
    }
    if ($("#favorite").length > 0) {
        $("#favorite").click(function () {
            if ($(this).hasClass("active")) {
                _gaq.push(['_trackEvent', 'pcDetail', '取消收藏']);
            } else {
                _gaq.push(['_trackEvent', 'pcDetail', '收藏']);
            }
        });
    }
    if (typeof selectedConditionInfos != "undefined") {
        var arrList = selectedConditionInfos.split("|");
        _gaq.push(['_setCustomVar', 1, 'City', arrList[0], 3]);
        _gaq.push(['_setCustomVar', 2, 'District', arrList[1], 3]);
        _gaq.push(['_setCustomVar', 3, 'Circle', arrList[2], 3]);
        _gaq.push(['_setCustomVar', 4, 'Price', arrList[3], 3]);
    }

    if (typeof (ServerDomain) != 'undefined' && $.cookie && (($.cookie(ServerDomain + "_ABTest") && $.cookie(ServerDomain + "_ABTest") != "None") || typeof (specialcityabtest) != 'undefined')) {
        var abCode = "";
        if ($.cookie(ServerDomain + "_ABTest") && $.cookie(ServerDomain + "_ABTest") != "None")
            abCode = $.cookie(ServerDomain + "_ABTest");
        if (typeof (specialcityabtest) != 'undefined') {
            if (abCode != "")
                abCode += ",";
            abCode += specialcityabtest;
        }
        _gaq.push(['_trackPageview', window.location.pathname + window.location.search + (window.location.search == "" ? "?" : "&") + "abtest=" + abCode]);
    }
    else {
        _gaq.push(['_trackPageview']);
    }

    _gaq.push(['_trackPageLoadTime']);
    if (typeof (trackTrans) != "undefined") { trackTrans(); }
    setTimeout(function () {
        var ga = document.createElement('script'); ga.type = 'text/javascript'; ga.async = true;
        //ga.src = ('https:' == document.location.protocol ? ' https://ssl' : ' http://www') + '.google-analytics.com/ga.js';
        ga.src = ('https:' == document.location.protocol ? 'https://' : 'http://') + 'stats.g.doubleclick.net/dc.js';
        var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(ga, s);
    }, 800);


    var _hmt = _hmt || [];
    (function () {
        var hm = document.createElement("script");
        hm.src = "//hm.baidu.com/hm.js?405c96e7f6bed44fb846abfe1f65c6f5";
        var s = document.getElementsByTagName("script")[0];
        s.parentNode.insertBefore(hm, s);
    })();




</script>



<!-- Google Code for Remarketing Tag -->
<script type="text/javascript">
    var google_tag_params = {
        ecomm_prodid: '90492',
        ecomm_pagetype: 'detail',
        ecomm_totalvalue: ''
    };
    var google_tag_params
    /* <![CDATA[ */
    var google_conversion_id = 964995166;
    var google_custom_params = window.google_tag_params;
    var google_remarketing_only = true;
    /* ]]> */
</script>
<script type="text/javascript">
    function asyncLoadScript(url, delayTime) {
        setTimeout(function () {
            var bd = document.createElement('script'); bd.type = 'text/javascript'; bd.async = true;
            bd.src = unescape(_bdhmProtocol + url);
            var b = document.getElementsByTagName('script')[0]; b.parentNode.insertBefore(bd, b);
        }, delayTime);
    }
    asyncLoadScript("www.googleadservices.com/pagead/conversion.js", 200);
</script>
<noscript>
    <div style="display:inline;">
        <img height="1" width="1" style="border-style:none;" alt="" src="//googleads.g.doubleclick.net/pagead/viewthroughconversion/964995166/?value=0&amp;guid=ON&amp;script=0" />
    </div>
</noscript>

<!--网易有道DSP跟踪代码 -->
<script type="text/javascript">var youdao_conv_id = 273380;</script>
<script type="text/javascript">
    (function () {
        setTimeout(function () {
            var a = document.createElement("script");
            var b = document.getElementsByTagName("script")[0];
            a.src = document.location.protocol + "//conv.youdao.com/pub/conv.js";
            a.async = true; a.type = "text/javascript"; b.parentNode.insertBefore(a, b)
        }, 1);
    }());
</script>






</body>
</html>
